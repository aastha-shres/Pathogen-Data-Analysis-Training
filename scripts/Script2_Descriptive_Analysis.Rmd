...
This script is all about tables and plots to report descriptive results of data generated using IDEXX Quantitray and Taqman Array Cards (TAC). You will practice merging different datasets, modifying/creating new variables, and using `ggplot()` to make plots with customized formatting.  
...
# the first argument should be the folder name, the second argument is the file name (including .csv)
...
write_csv(tac_prev, here("outputs", "tac_prevalence.csv")) # create "outputs" folder before saving
```

#### Visualize prevalence of TAC pathogen targets
Here is a basic scatterplot created using `ggplot()`. 
```{r plot.tac.prevalence}
# Workshop Activity: Run the code as is first. Then uncomment the coord_flip() line (delete the #) and rerun. Notice how the plot changes.
tac_prev %>% # Start with prevalence dataset you created above, then pipe it into the ggplot() function
...
# Save plot
ggsave( here("figures", "pathogen_prevalence_scatterplot.png"), dpi = 300, width = 7, height = 5, unit = "in")
```
...
# Workshop Task: ADD CODE HERE to take a peak at the top 10 pathogens by prevalence
...
# Workshop task: Run the code, then uncomment the facet_wrap() line of code (remove #) to see how the plot changes
tac_prev_type %>% 
  filter(Category != "Control" & Category != "ARG") %>% # remove control samples & ARG targets
  ggplot(aes(x = reorder(Target, prevalence), # plot x (Target) in order of prevalence values
  ...
  labs(title = "Pathogen Prevalence by Sample Type (TAC)", 
       x = "Target", y = "Prevalence", color = "Sample Type") +
  theme_bw(base_size = 11) + # use this for set themes - e.g. theme_minimal(), theme_classic(), etc.
...
# Save plot when you are ready
ggsave(here("figures", "pathogen_prevalence_scatterplot_faceted.png"), 
...
tac_burden %>%
  ggplot(aes(x =  , fill =  )) + # ADD CODE HERE
  geom_histogram(binwidth = 1, alpha = 0.8) + # test out what happens when you change binwidth 
  labs(title = "Pathogen Burden (TAC)", fill = "Sample Type", x = "# Pathogens", y = "Count") + 
  theme_bw() 

# Save plot
ggsave( here("figures", "pathogen_burden_histogram.png"), # specify folder and filename and file type (.png)
        dpi = 300, width = 7, height = 5, unit = "in")
```
...
# Save plot
ggsave( here("figures", "pathogen_heatmap.png"), dpi = 300, width = 5, height = 7, unit = "in")
```


## Culture Data Analysis
First let's calculate the mean prevalence by sample type using `summarise()`. Practice rounding the values you calculate with mean by adding `round()` around the `mean()` function, rounding to 3 decimals.  
```{r idexx.prevalence}
# Workshop Activity: Modify the code below to round each new variable to 3 decimals. 
microbio_clean %>%
  group_by(sample_type) %>% # group by sample type
  summarize(
    #specify na.rm to remove NAs from the calcuation of ec_detect mean value
    ecoli_positive = round(mean(ec_detect, na.rm=T), 3),
...
```{r esbl.plot}
# Workshop Task: Run the ggplot() code as is, then alter the specifications vjust, size, and color in geom_text() to see how it changes the labels.
microbio_clean %>%
...
# Save plot when you are ready
ggsave( here("figures", "ESBL_barchart.png"), dpi = 300, width = 5, height = 3, unit = "in")
```
...
```{r join.data}
# Reshape data from long to wide (one row per sample, with each target being in a different column rather than each target being a new row for each sample)
tac_household <- tac_clean %>% 
  select(household_id, sample_type, Target, detect) %>% 
  pivot_wider(names_from = Target, values_from = detect)  

# Compare the tac_clean dataset to the tac_household dataset - what difference do you notice? 
head(tac_clean)
head(tac_household)
```
...
```{r concordance.ec}
tac_wide %>%
  select(ec_detect, `E. coli O157`) %>% # add ` ` around variable name because of space in name
...
tac_wide %>%
  select(esbl_ecoli_detect, `blaCTX-M`) %>% # add ` ` around variable name because of - in name
  tabyl( ) %>% # ADD CODE HERE
  adorn_percentages( ) %>% # add row percent
  adorn_pct_formatting( ) # add digits here
```
...
```{r arg.heatmap.data}
# Join micro and tac data again, but this time keep in long format because we want to plot by Target and Category 
...
heatmap_dat %>% # Pipe in dataset to ggplot()
  ...
  # set colors for ESBL+ samples, ESBL- samples, and no ARG detects
  scale_fill_manual( 
    values = c(
  ...
  labs(title = "ARG Detections by Household", x = "Household Samples", y = "ARG Target") +
  facet_wrap(~sample_type, scales = "free_x", nrow = 3) + # stratify plot by sample type with plots in 3 rows
  theme_bw(base_size = 11) +
  theme(axis.text.x = element_blank(),
        axis.ticks = element_blank())

# save plot
ggsave( here("figures", "ARG_ESBL_detects.png"), dpi = 300, width = 7, height = 5, unit = "in")
```
...
# save plot
ggsave( here("figures", "pathogen_count_moisture.png"), dpi = 300, width = 7, height = 5, unit = "in")
```
